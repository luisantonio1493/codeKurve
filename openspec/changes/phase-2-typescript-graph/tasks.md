...
**PR4b-2 apply-time notes**: (1) `expected_relationships.json` (task 4b.4's original suggestion) was dropped in favor of inline Rust assertions in the acceptance test — parsing it would need `serde_json`, which is deliberately scoped to PR5b (task 5b.2) only; hand-rolling JSON parsing just for this one fixture isn't worth the complexity. (2) The fixture cannot exercise `RelationshipKind::References` — no code path in `extract.rs`/`resolve.rs` emits it in this phase (pre-existing gap, not introduced here). (3) `Implements` never resolves to a `relationships` row in this codebase yet (no `Interface` symbols are extracted anywhere) — used deliberately as the fixture's "real" unresolved-reference case instead of relying on the known default-export/namespace-import gaps.

- [x] 4b.4 `crates/codekurve-analysis/tests/fixtures/ts-graph/project/`: new multi-file fixture (`base.ts`/`utils.ts`/`app.ts`) covering imports, exports, contains, extends (inherits), implements (as the unresolved case), calls (same-file + cross-file), constructs, and an unresolved import — see apply-time note (1) on `expected_relationships.json`. (PR4b-2)
- [x] 4b.5 Test (acceptance gate, §34.2-34.3): `crates/codekurve-analysis/tests/relationship_graph_fixture.rs` (new) — runs `analyze`+`resolve` over the fixture, asserts relationship counts by kind, one specific cross-file edge (`app.ts::Foo.run`'s `helper()` call -> `utils.ts::helper`, High/Resolved), and both unresolved references (`implements IFoo`, `import ... from './nonexistent'`) recorded with `candidate_count: 0`. (PR4b-2)
...
- [x] 5a.2 `crates/codekurve-store/src/traverse.rs` (new): `load_adjacency(project_id) -> HashMap<SymbolId, Vec<Edge>>`; `bfs()` forward (trace) and reverse (impact) with depth/node/edge/time caps + `Truncated{reason}`. (PR5a-2)
...
- [x] 5a.5 Test: `traverse.rs` — path found within depth (scenario "Path found within depth"); depth exceeded → `truncated:true, reason:max_depth` (scenario "Depth limit exceeded"); reverse impact truncation (scenario "Impact truncation"). (PR5a-2)

## PR5b: CLI query commands (base: PR5a-2, per PR5a's apply-time split note above)

**Split note (apply-time)**: the full scope (CommandError + cli.rs parsing + 6 command fns + main.rs dispatch + test file) measured ~460 changed lines combined, over the 400-line budget — split into two chained sub-PRs, mirroring the PR3a/b/c, PR4a-1/PR4a-2, PR4b-1/PR4b-2, and PR5a-1/PR5a-2 precedent: **PR5b-1** (`CommandError`, `cli.rs` parsing minus `--depth`, `references`/`callers`/`callees`/`implementations`, `main.rs` dispatch for those four, ~436 changed lines) and **PR5b-2** (base: PR5b-1; `--depth` + `trace`/`impact` + their `main.rs` dispatch, ~246 changed lines). `--depth` was deliberately deferred to PR5b-2 rather than shipped in PR5b-1's "core plumbing" as originally suggested — landing it in PR5b-1 unused (no command reads it yet) would leave a dead struct field in a binary crate, tripping `clippy -D warnings`' `dead_code` lint; landing it alongside `trace`/`impact` (its only two readers) keeps every commit warning-clean. Tests were split the same way: PR5b-1 covers qualified-name resolution + confidence/provenance + `--min-confidence` filtering + bare-name ambiguity (exit 6); PR5b-2 adds the missing-index exit-4 case, the JSON envelope shape check, and a `trace`/`impact` smoke test.

- [x] 5b.1 `crates/codekurve/src/commands.rs`: `CommandError{code:u8, message:String}` + `impl From<String> for CommandError` (code 1) — lands here only, per design. (PR5b-1)
- [x] 5b.2 `crates/codekurve/Cargo.toml`: add `serde_json = "1"` (JSON envelope, no existing dep covers it). (PR5b-1)
- [x] 5b.3 `crates/codekurve/src/cli.rs`: extend hand-rolled `Args` parsing for `--depth`, `--min-confidence`, `--json`, `--symbol-id`/`--symbol-name`, `--limit`/`--offset` (no clap). (`--depth` in PR5b-2, per split note above; the rest in PR5b-1)
- [x] 5b.4 `crates/codekurve/src/commands.rs`: add `references`/`callers`/`callees`/`implementations`/`trace`/`impact` fns using PR5a repo/traverse fns; bare-name ambiguity → `CommandError{code:6}` listing all candidates (spec "Ambiguous name lookup"); missing index → `CommandError{code:4}` (spec "Query before first index"). (references/callers/callees/implementations in PR5b-1; trace/impact in PR5b-2)
- [x] 5b.5 `crates/codekurve/src/commands.rs`: `--json` envelope `{schema_version, project, result, warnings, truncated}` via `serde_json` (spec "JSON envelope shape"). (PR5b-1; trace/impact's JSON result shape added in PR5b-2)
- [x] 5b.6 `crates/codekurve/src/main.rs`: dispatch the 6 new commands; map `CommandError.code` to `ExitCode`. (four commands in PR5b-1; trace/impact in PR5b-2)
- [x] 5b.7 Test: `crates/codekurve/tests/graph_queries.rs` (new) — callers returns confidence/provenance; `--min-confidence high` filters Low out; bare-name ambiguity exits 6 with both candidates listed; qualified name resolves exit 0; query before index exits 4; `--json` output has all 5 envelope fields. (first four scenarios in PR5b-1; missing-index/JSON-envelope/trace-impact scenarios in PR5b-2)

**Chain complete**: this is the final PR of the phase-2-typescript-graph 7-PR (9, after live splits) chain. Every task in this file (PR1 through PR5b) is now `[x]`. Merging the full chain to `main` is a separate decision for the user to make explicitly — not performed as part of this apply run.
