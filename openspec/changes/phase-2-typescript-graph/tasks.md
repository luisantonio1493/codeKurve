...
- [ ] 4a.3 `resolve.rs`: `resolve()` — import/export edge resolution + call/construct confidence tiers (§20.4): Exact (same-file), High (single cross-file candidate), Low (multi-candidate — one edge per candidate, never pick first), zero-candidate → `UnresolvedReference`. (PR4a-2)
...
- [ ] 4a.6 Test: table test — cross-file call resolves High to the single candidate; multi-candidate cross-file calls → Low-confidence/Heuristic-provenance edges, no unresolved row (scenario "Multi-candidate call is not unresolved" / "Ambiguous member call"); 0-candidate call → `UnresolvedReference` with `candidate_count=0` (scenario "Zero-candidate import" principle). (PR4a-2)
